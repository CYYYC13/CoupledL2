...
class TestTop_4L2L3()(implicit p: Parameters) extends LazyModule {
...
object TestTop_4L2L3 extends App {
  val config = new Config((_, _, _) => {
    case L2ParamKey => L2Param(
      clientCaches = Seq(L1Param(aliasBitsOpt = Some(2))),
      // echoField = Seq(DirtyField())
  ...
  val top = DisableMonitors(p => LazyModule(new TestTop_4L2L3()(p)))(config)
